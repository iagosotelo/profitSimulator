...
ax.plot(df["día"], df["saldo_usuario"], label="Saldo Usuario")
ax.plot(df["día"], df["comision_acumulada"], label="Comisiones acumuladas")
for i, r in enumerate(referidos):
    ax.plot(df["día"], [s[i] for s in df["saldos_referidos"]], label=f"Saldo referido nivel {r['nivel']}")
ax.set_xlabel("Día")
ax.set_ylabel("Saldo (€)")
ax.set_title("Evolución diaria de saldo y comisiones con cuantificaciones")
ax.legend()
ax.grid(True)
st.pyplot(fig)
